...
public class ImagesActivity extends AppCompatActivity implements PictureTaker, ImageImporter {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            // It does not return the file itself. Extract the URI from the intent
            // and process it.
    ...
    private void importUriImage(@NonNull Uri uri) {
        try {
            ...
            e.printStackTrace();
            Log.e(TAG, "Error: " + e.getMessage() + "Could not open URI: " + uri.toString());
        }
    }
    ...
    public void importImage() {
        // start an intent that will allow a user to select a file using the system's browser
